...
export default function LikeCard({ content, navigation, likeTip, setLikeTip }) {
  const goDetailPage = () => {
    navigation.navigate("DetailPage", { idx: content.idx });
  };

  const removeLikeTip = async (idx) => {
    let userId;
    if (Platform.OS === "ios") {
    ...
    firebase_db
      ...
      .then(() => {
        Alert.alert("삭제 완료");
        const result = likeTip.filter((_, i) => i !== idx);
        setLikeTip(result);
      });
  };
    ...
    <TouchableOpacity style={styles.card} onPress={() => goDetailPage()}>
      ...
      <View style={styles.cardText}>
        ...
        <View style={styles.buttonGroup}>
          ...
          <TouchableOpacity
            style={styles.button}
            onPress={() => removeLikeTip(content.idx)}
          >
